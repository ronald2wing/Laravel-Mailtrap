 ...
 *
 * Provides seamless integration with Mailtrap's Email Sending API
 * for Laravel applications. This transport handles email delivery
 * through Mailtrap's REST API with comprehensive feature support.
 *
...
class MailtrapTransport extends AbstractTransport
{
    ...
    protected function buildApiPayload(Email $email, Envelope $envelope): array
    {
        ...
        $this->addCcAddresses($email, $payload);
        $this->addBccAddresses($email, $payload);
        $this->addEmailBody($email, $payload);
        $this->addAttachments($email, $payload);
        $this->addCategory($email, $payload);
        $this->addCustomHeaders($email, $payload);
    ...
    /**
     * Add CC addresses to the payload
     *
     * @param  Email  $email  The email
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addCcAddresses(Email $email, array &$payload): void
    {
        $cc = $email->getCc();
    ...
    /**
     * Add BCC addresses to the payload
     *
     * @param  Email  $email  The email
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addBccAddresses(Email $email, array &$payload): void
    {
        $bcc = $email->getBcc();
    ...
    /**
     * Add email body content to the payload
     *
     * @param  Email  $email  The email
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addEmailBody(Email $email, array &$payload): void
    {
    ...
    /**
     * Add category to the payload if available
     *
     * @param  Email  $email  The email
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addCategory(Email $email, array &$payload): void
    {
     ...
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addCustomHeaders(Email $email, array &$payload): void
    {
     ...
     * @param  array<string, mixed>  $payload  The payload to modify
     */
    protected function addReplyTo(Email $email, array &$payload): void
    {
